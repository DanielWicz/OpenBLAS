...
- Benchmark catalogue (keep binaries/results untracked):
  ...
  - `benchmark/numa_copy_bench.c`: NUMA-aware memcpy microbench. Measures local, remote-read, remote-write, and bidirectional bandwidth with thread pinning; exercises cache/L3/RAM and cross-socket paths to spot multi-instance bottlenecks under `USE_TLS=1`. Build: `gcc -O2 -fopenmp -lnuma -I. benchmark/numa_copy_bench.c -o benchmark/numa_copy_bench`; run with `OMP_NUM_THREADS=...` (tune `BENCH_SIZES`, `BENCH_THREADS`, `BENCH_CASES`, `BENCH_NODEA/B`).
